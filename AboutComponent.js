    ...
    return (
        ...
        {comments.map(coment =>{ 
            return (
                <div key={comment.id}>
                    <p>{comment.text} <br />
                    {comment.author} - {new Intl.DateTimeFormat('en-US' {year: 'numeric', month: 'short', day: '2-diget'}).format(new Date(Date.parse(coment.date)))}
                    </p>
                </div>
             );
            })}
    ...
    if (this.props.campsite){
        return (
            <div className="row">
                ...
                {this.renderCampsite(this.props.camments)}
            </div>
        );
    }else {
        return(
